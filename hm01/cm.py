...
class ClusterIgnoreFilter:
    ...
    def __call__(
        self,
        cluster: IntangibleSubgraph,
        global_graph: Graph,
    ) -> bool:
        if self.ignore_trees and cluster.is_tree_like(global_graph) == 1:
            return True
        if self.ignore_smaller_than > 0 \
            and cluster.n() < self.ignore_smaller_than:
            return True
        return False
...
def main(
    ...
    ignore_trees: bool = typer.Option(
        False,
        "--ignore-trees",
        "-x",
    ),
    ignore_smaller_than: int = typer.Option(
        0,
        "--ignore-smaller-than",
        "-s",
    ),
):
    """Connectivity-Modifier (CM).
    ...
    if clusterer_spec == ClustererSpec.leiden:
        assert resolution != -1, \
            "Leiden requires resolution"
        clusterer: Union[LeidenClusterer, IkcClusterer] = \
            LeidenClusterer(resolution)
    ...
    log.info("parsed cluster filter", filterer=filterer)
    ...
    if not existing_clustering:
        log.info(
            "running first round of clustering before algorithm-g",
            clusterer=clusterer,
        )
